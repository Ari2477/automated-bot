...
module.exports.run = async function({ api, event, args }) {
  ...
  try {
    ...
    allThreads.forEach((thread, index) => {
      msg += `${index + 1}. ${thread.name || "Unnamed Group"}\n`;
    ...
    return api.sendMessage(`Error fetching groups: ${error.message || error}`, threadID);
  }
};
  ...
  try {
    ...
    if (approvalMode) {
      ...
      return api.sendMessage(
        `You have joined ${threadName || "Unnamed Group"}. Check your inbox if it doesn't appear immediately.`,
        threadID,
        messageID
      );
    }
