...
function formatFont(text) {
    ...
    let formatted = "";
    for (const c of text) formatted += fontEnabled && c in fontMapping ? fontMapping[c] : c;
    return formatted;
}
...
module.exports.getStartTimestamp = async (forceReset = false) => {
    try {
        const startTimeStr = await fsPromises.readFile('./time.txt','utf8');
        if(forceReset){
            const now = Date.now();
...
module.exports.saveStartTimestamp = async (timestamp) => {
    try { await fs.writeFile('./time.txt', timestamp.toString()); }
    catch(e){ console.error("[UPTIME] Failed to save start timestamp:", e.message); }
};
...
module.exports.run = async ({ api, event }) => {
    ...
    const osInfo = { platform: os.platform(), arch: os.arch(), cpus: os.cpus().length };
